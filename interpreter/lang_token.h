...
public:
  Token() :
      Token(nullptr, NA_TYPE)
  {
  }
  Token(char const* text, TokenIdType const type) :
      Token(text, 0, type)
  {
  }
  Token(char const* text, size_t len, TokenIdType const type) :
      text(text), len(len), type(type)
  {
  }
